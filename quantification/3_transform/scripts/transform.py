...
def main(input_file, output_file, window_size, sample_list_file=None):
    ...
    if sample_list_file is not None:
        sys.stderr.write(f"Loading sample list from {sample_list_file}\n")

        allowed_samples = set()

        with open(sample_list_file, "r") as sf:
    ...
    if allowed_samples is not None:
        ...
        sys.stderr.write(
            f"Retaining {len(sample_names)} / {len(all_sample_names)} samples\n"
        )

        if len(sample_names) == 0:
            raise ValueError("No matching samples found in sample list")
    else:
        keep_indices = list(range(len(all_sample_names)))
    ...
    for line in lines[1:]:
        ...
        mean_edit = (
            float(np.nanmean(editing_levels))
            if n_missing < len(editing_levels)
            else np.nan
        )
